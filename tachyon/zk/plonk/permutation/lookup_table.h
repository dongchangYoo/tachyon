...
class LookupTable {
 public:
  ...
  static LookupTable Construct(
      const size_t size,
      math::UnivariateEvaluationDomain<F, MaxDegree>* domain) {
    // The omega is gᵀ with order 2ˢ where modulus = 2ˢ * T + 1.
    std::vector<F> omega_powers =
        domain->GetRootsOfUnity(MaxDegree + 1, domain->group_gen());

    // The 𝛿 has order T where modulus = 2ˢ * T + 1.
    F delta = GetDelta();
    ...
    for (size_t i = 1; i < size; ++i) {
      ...
      for (size_t j = 0; j <= MaxDegree; ++j) {
        rows[j] = lookup_table[0][j] * delta_iter;
      }
